...
(setq ret "")
(defun get-code ()
  (let ((dir (concat (getenv "HOME") "/share/Projects/Code/"))
        (ret ""))
    (cl-loop for element in (compat-directory-files dir)
             do (setq ret (concat ret "\tdirectory = \"" dir element "\"\n"))
             finally return ret)))
(defun get-doc ()
  (let ((dir (concat (getenv "HOME") "/share/Projects/Doc/"))
        (ret ""))
    (cl-loop for element in (compat-directory-files dir)
             do (setq ret (concat ret "\tdirectory = \"" dir element "\"\n"))
             finally return ret)))
(concat (get-code)
        (get-doc)
        (cl-loop for element in '("/share/org" "/share/notes" "/share/notes/ZK")
                 do (setq ret (concat ret "\tdirectory = \"" (getenv "HOME") element "\"\n"))
                 finally return ret))
#+end_src

...
#+STARTUP: fold
#+SETUPFILE: ~/.doom.d/org-styles/readtheorg_inline/readtheorg_inline.theme
...
Keybindings to help manage text in buffers. To activate Avy, check the various =avy-= commands under (~SPC j~):
| Key     | Action                   |
|---------+--------------------------|
...
*** STOPPED Require Org-Protocol (Broken)

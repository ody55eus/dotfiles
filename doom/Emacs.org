...
#+name: mymail
#+begin_src sh
gpg --list-secret-keys | head -n6 | tail -n1 | cut -d'<' -f2 | cut -d'>' -f1
#+end_src

#+begin_src emacs-lisp :tangle ./.doom.d/config.el :noweb yes
...
*** MacOS Only Packages
- [[https://github.com/benmaughan/spotlight.el][Spotlight]] uses MacOS system's search engine to find files.
...
***** Windows
:Source:
Adapted from [[doom-module:ui/hydra][Doom Emacs (:ui hydra module)]]
:END:
